...
import { LoginDto, SignUpDto, UpdateTodoDto } from '@todo-example-app-lib/shared';
...
export class AppFacade {
  readonly user = {
    dispatchers: {
      login: (dto: LoginDto) => {
        this._store.dispatch(UserActions.userLogin({ dto }));
      },
      signUp: (dto: SignUpDto) => {
        this._store.dispatch(UserActions.userSignUp({ dto }));
      },
      getProfile: () => {
      ...
      updateProfilePic: (file: File) => {
        this._store.dispatch(UserActions.updateProfilePic({ file }));
      },
    },
    actionListeners: {
      login: {
